          ...
          {step === 2 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex flex-wrap gap-2.5">
                {["Python", "Data Analysis", "Communication", "Problem Solving", "MS Excel", "ReactJS", "AutoCAD", "SQL Basics", "Accounting", "Customer Support", "Digital Marketing"].map(skill => (
                  <button
                    key={skill}
                    onClick={() => toggleSelection(profile.skills, skill, 'skills')}
                    className={`flex items-center space-x-2 py-2 px-3.5 rounded-full border text-xs font-medium transition-all ${profile.skills.includes(skill) ? 'bg-blue-50 border-primary text-primary font-semibold shadow-xs' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'}`}
                  >
                    {profile.skills.includes(skill) && <CheckCircle size={14} />}
                    <span>{skill}</span>
                  </button>
                ))}
              </div>
