...
def get_recommendations(payload: CandidateInput, db: Session = Depends(get_db)):
    ...
    if payload.candidate_id:
        ...
        if db_cand:
            ...
            # Filter skills based on verification status
            raw_skills = [s.strip() for s in db_cand.skills.split(",") if s.strip()] if not payload.skills else payload.skills
            from app.db.helpers import get_skill_status
            verified_skills = []
            for s in raw_skills:
                status = get_skill_status(db_cand, s)
                if status in ["verified", "verified (fallback)"]:
                    verified_skills.append(s)
            cand_dict["skills"] = verified_skills
