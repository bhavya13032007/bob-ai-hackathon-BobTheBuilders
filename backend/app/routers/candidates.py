...
def get_candidate(candidate_id: str, db: Session = Depends(get_db)):
    ...
    from app.db.helpers import get_candidate_skill_verifications
    
    cand_dict = {k: v for k, v in cand.__dict__.items() if not k.startswith("_")}
    cand_dict["skill_verifications"] = get_candidate_skill_verifications(cand)
...
def create_or_update_candidate(payload: CandidateCreateUpdate, db: Session = Depends(get_db)):
    ...
    from app.db.helpers import sync_skills_fallback, get_candidate_skill_verifications
    sync_skills_fallback(cand, db)
