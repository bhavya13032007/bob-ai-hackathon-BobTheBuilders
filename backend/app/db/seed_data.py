...
from app.db.models import Candidate, Company, Internship, Certificate, Application, Notification, UserPreference, SkillQuiz
    ...
    try:
        ...
        quizzes = [
            SkillQuiz(
                ...
                difficulty="easy",
                language_variants=json.dumps({"hi": {"question_text": "पायथन में फ़ंक्शन को परिभाषित करने के लिए निम्न में से किसका उपयोग किया जाता है?", "options": ["func", "def", "function", "lambda"]}})
            ),
            SkillQuiz(
                id="q_py_2", skill_name="python",
                question_text="What data type is the object L = [1, 23, 'hello', 1]?",
                options=json.dumps(["List", "Dictionary", "Tuple", "Array"]),
                correct_option_index=0,
                difficulty="easy",
                language_variants=json.dumps({"hi": {"question_text": "ऑब्जेक्ट L = [1, 23, 'hello', 1] किस डेटा प्रकार का है?", "options": ["सूची (List)", "शब्दकोश (Dictionary)", "टुपल (Tuple)", "सरणी (Array)"]}})
            ),
            SkillQuiz(
                id="q_py_3", skill_name="python",
                question_text="How do you insert comments in Python code?",
                options=json.dumps(["//", "/* */", "#", "--"]),
                correct_option_index=2,
                difficulty="easy",
                language_variants=json.dumps({"hi": {"question_text": "आप पायथन कोड में टिप्पणियाँ (comments) कैसे डालते हैं?", "options": ["//", "/* */", "#", "--"]}})
            ),
            SkillQuiz(
                id="q_da_1", skill_name="data analysis",
                question_text="What does 'SQL' stand for?",
                options=json.dumps(["Structured Question Language", "Strong Question Language", "Structured Query Language", "Sequential Query Language"]),
                correct_option_index=2,
                difficulty="easy",
                language_variants=json.dumps({"hi": {"question_text": "'SQL' का पूर्ण रूप क्या है?", "options": ["स्ट्रक्चर्ड क्वेश्चन लैंग्वेज", "स्ट्रॉन्ग क्वेश्चन लैंग्वेज", "स्ट्रक्चर्ड क्वेरी लैंग्वेज", "सीक्वेंशियल क्वेरी लैंग्वेज"]}})
            ),
            SkillQuiz(
                id="q_da_2", skill_name="data analysis",
                ...
                correct_option_index=1,
                difficulty="medium",
                language_variants=json.dumps({"hi": {"question_text": "डेटा हेरफेर के लिए मुख्य रूप से किस पायथन लाइब्रेरी का उपयोग किया जाता है?", "options": ["नमपाय (NumPy)", "पंडास (Pandas)", "मैटप्लॉटलिब (Matplotlib)", "साइकिट-लर्न (Scikit-Learn)"]}})
            ),
            SkillQuiz(
                id="q_da_3", skill_name="data analysis",
                question_text="What is the process of detecting and correcting inaccurate records from a dataset called?",
                options=json.dumps(["Data Mining", "Data Cleaning", "Data Exploration", "Data Integration"]),
                correct_option_index=1,
                difficulty="easy",
                language_variants=json.dumps({"hi": {"question_text": "डेटासेट से गलत रिकॉर्ड का पता लगाने और उन्हें सही करने की प्रक्रिया को क्या कहा जाता है?", "options": ["डेटा माइनिंग", "डेटा क्लीनिंग", "डेटा एक्सप्लोरेशन", "डेटा इंटीग्रेशन"]}})
            ),
            SkillQuiz(
                id="q_comm_1", skill_name="communication",
                question_text="Which of these is NOT a form of non-verbal communication?",
                options=json.dumps(["Eye contact", "Body language", "Writing an email", "Facial expressions"]),
                correct_option_index=2,
                difficulty="easy",
                language_variants=json.dumps({"hi": {"question_text": "इनमें से कौन सा गैर-मौखिक (non-verbal) संचार का रूप नहीं है?", "options": ["आँख मिलाना", "शारीरिक हावभाव", "ईमेल लिखना", "चेहरे के भाव"]}})
            ),
            SkillQuiz(
                id="q_comm_2", skill_name="communication",
                question_text="What does 'active listening' involve?",
                options=json.dumps(["Interrupting to show agreement", "Focusing completely on the speaker", "Planning what to say next", "Looking away frequently"]),
                correct_option_index=1,
                difficulty="medium",
                language_variants=json.dumps({"hi": {"question_text": "'सक्रिय श्रवण' (active listening) में क्या शामिल है?", "options": ["सहमति दिखाने के लिए बीच में बोलना", "बोलने वाले पर पूरी तरह ध्यान केंद्रित करना", "यह योजना बनाना कि आगे क्या कहना है", "बार-बार दूर देखना"]}})
            ),
            SkillQuiz(
                id="q_comm_3", skill_name="communication",
                question_text="In an email, what does 'BCC' stand for?",
                options=json.dumps(["Blind Carbon Copy", "Blank Carbon Copy", "Behind Carbon Copy", "Bottom Carbon Copy"]),
                correct_option_index=0,
                difficulty="easy",
                language_variants=json.dumps({"hi": {"question_text": "ईमेल में, 'BCC' का क्या अर्थ है?", "options": ["ब्लाइंड कार्बन कॉपी", "ब्लैंक कार्बन कॉपी", "बिहाइंड कार्बन कॉपी", "बॉटम कार्बन कॉपी"]}})
            )
        ]
        db.add_all(quizzes)
        db.commit()
