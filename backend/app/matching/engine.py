...
def compute_tfidf_similarity(cand_text: str, req_text: str) -> float:
    try:
        if not cand_text.strip() or not req_text.strip():
            return 0.3
        cand_tokens = set(clean_text(cand_text).split())
        req_tokens = set(clean_text(req_text).split())
        if not cand_tokens or not req_tokens:
            return 0.3
        intersection = cand_tokens.intersection(req_tokens)
        union = cand_tokens.union(req_tokens)
