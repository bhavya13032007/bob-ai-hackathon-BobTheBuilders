...
const DashboardPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {(() => {
        const unverifiedCount = Object.values(candidateData?.candidate?.skill_verifications || {}).filter(s => s === 'unverified').length;
        if (unverifiedCount > 0) {
          return (
            <div className="mb-8">
              <Banner
                variant="warning"
                title={`${unverifiedCount} skills need a quick check`}
                message="Recommendations will improve once you verify your claimed skills. Verified skills boost your match score!"
                action={
                  <Link to="/wizard" className="inline-block mt-1 text-red-700 font-bold hover:underline text-xs">
                    Take Quiz Now →
                  </Link>
                }
              />
          ...
          <div className="bg-gradient-to-br from-primary to-[#0f3d6e] rounded-3xl p-6 text-white shadow-sm relative overflow-hidden">
            <div className="relative z-10 space-y-2.5">
              <div className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-secondary">
                <HeartHandshake className="w-3.5 h-3.5" /> Loopin Saathi
              </div>
